...
//            finalColor += lightInput.rgb * diffColor.rgb + lightSpecColor * specColor;
        ...
        #endif
        #ifdef EMISSIVEMAP
            // Урок: закомментированный оригинал
            //finalColor += cMatEmissiveColor * Sample2D(EmissiveMap, iTexCoord.xy).rgb;

            // Урок: добавлено
            // Домножаем на диффузный цвет
            finalColor += cMatEmissiveColor * Sample2D(EmissiveMap, iTexCoord.xy).rgb * diffColor.rgb;
        #else
            finalColor += cMatEmissiveColor;
        #endif
        ...
        float3 viewDir = normalize(cCameraPosPS - iWorldPos.xyz);
        ...
        // Проверяем, нужно ли использовать карту подсветки
            ...
            finalColor += Sample2D(EnvMap, iTexCoord.xy).rgb * cRimColor * rimFactor;
        #else
